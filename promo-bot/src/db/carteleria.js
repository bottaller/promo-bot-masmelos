...
async function crearCarteleria({ fotoFileId, tipo, cantidadCopias, usuarioId, usuarioNombre, usuarioTelegramId }) {
  const { rows } = await pool.query(
    `insert into bot.carteleria (foto_file_id, tipo, cantidad_copias, usuario_id, usuario_nombre, usuario_telegram_id)
     values ($1,$2,$3,$4,$5,$6)
     returning id`,
    [fotoFileId, tipo, cantidadCopias ?? null, usuarioId ?? null, usuarioNombre ?? null, usuarioTelegramId]
  );
  return rows[0].id;
}
