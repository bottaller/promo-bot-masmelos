...
- **Autorización por comando:** cada comando declara su área (`requiereArea('calidad')`).
  ...
  - **Tesorería:** `/carga` (carga nocturna del **libro** de Sigma + la **liquidación** de MP — el bot reconoce cada archivo solo; Talo se baja sola por API a las 21:00; reemplazó a `/libro`. Desde la migración 040 el comando **lo comparte con Retiros** y el permiso pasó a ser **por documento**, no por comando: el libro y las liquidaciones siguen siendo admin-only, ver `src/lib/documentos-carga.js`), `/flujos` (recibe el Excel de Sigma y devuelve el HTML del flujo del dinero — corre el motor Python, ver §6 y [areas/tesoreria.md](areas/tesoreria.md)) y `/cierre` (cierre diario en dos tiempos: los saldos a la tarde, la conciliación contra el libro a las 08:00, ver [conciliacion.md](conciliacion.md)).
  ...
  - **Retiros** (migración 040): `/carga` — sube la **PLANILLA RETIRA** de Moreno, que alimenta la pantalla `/tv_recepcion` del sitio (tabla `public.retiros`, parser en `src/lib/retiros-excel.js`). Es la misma puerta que usa Tesorería: entra por `/carga` pero solo puede subir su planilla, porque el permiso viaja con el documento. Área propia y no dentro de Depósito a propósito: quien mantiene la planilla no tiene por qué acceder al resto de los comandos de Depósito.
...
- **Rol "Sistemas"** (migración 019): ve y usa casi **todos los comandos** — todas las áreas más
  `/usuarios`, `/actartic` y `/avisos` (gateados con `requiereAdminOSistemas()`,
  ...
  **Tesorería queda totalmente afuera** (a pedido): ni la ve en el menú ni puede correr ninguno de
  sus comandos, ni siquiera `/reportecierre` (que es `requiereAdmin()` puro). `/carga` es el caso
  especial desde la migración 040: la puerta se abrió porque el comando ahora también lo usa
  Retiros, pero adentro el libro y las liquidaciones siguen exigiendo **admin real**, así que
  "sistemas" entra al wizard y solo puede subir la planilla de retiros.
