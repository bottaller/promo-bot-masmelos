...
- **Autorización por comando:** cada comando declara su área (`requiereArea('calidad')`).
  ...
  - **Retiros** (migración 042): `/carga` — sube la **PLANILLA RETIRA** de Moreno, que alimenta la pantalla `/tv_recepcion` del sitio (tabla `public.retiros`, parser en `src/lib/retiros-excel.js`). Es la misma puerta que usa Tesorería: entra por `/carga` pero solo puede subir su planilla, porque el permiso viaja con el documento. Área propia y no dentro de Depósito a propósito: quien mantiene la planilla no tiene por qué acceder al resto de los comandos de Depósito.
...
- **Rol "Sistemas"** (migración 019): ve y usa casi **todos los comandos** — todas las áreas más
  ...
  **Tesorería queda totalmente afuera** (a pedido): ni la ve en el menú ni puede correr ninguno de
  ...
  especial desde la migración 042: la puerta se abrió porque el comando ahora también lo usa
