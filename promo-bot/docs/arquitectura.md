...
| D9 | **El "rol" de una persona = las áreas a las que pertenece** (N:N en `usuario_area`) | YAGNI: no hace falta una tabla de roles aparte. Hoy hay 4 áreas con comandos reales (Compras, Calidad, Tesorería, Caja Central) y una persona puede tener varias. Los admins ven todo. **Corolario:** un comando pertenece a **una sola** área — si dos lo registraran, `bot.command()` correría dos veces. Crear un rol = migración que lo siembra en `bot.areas` + carpeta en `src/areas/` + sumarlo a la lista de `index.js`. |
