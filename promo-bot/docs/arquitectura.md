...
- **Autorización por comando:** cada comando declara su área (`requiereArea('calidad')`).
  ...
  - **Tesorería:** `/carga` (admin; carga nocturna del **libro** de Sigma + la **liquidación** de MP — el bot reconoce cada archivo solo; Talo se baja sola por API a las 21:00; reemplazó a `/libro`), `/flujos` (recibe el Excel de Sigma y devuelve el HTML del flujo del dinero — corre el motor Python, ver §6 y [areas/tesoreria.md](areas/tesoreria.md)) y `/cierre` (cierre diario en dos tiempos: los saldos a la tarde, la conciliación contra el libro a las 08:00, ver [conciliacion.md](conciliacion.md)).
