...
_COL = {"cajas": 0, "central": 0, "central_bis": 0, "tesoreria": 0,
        ...
        "caja_dolar_tes": 4}  # dólares en tránsito, un eslabón antes del destino Caja Dolares
_COL_DEST = 5
...
def _arbol(salidas: pd.DataFrame, autoriz: dict[tuple, str]) -> list[dict]:
    ...
    for (dnodo, dlabel, cat), g in salidas.groupby(["destino_id", "destino", "categoria"]):
        filas = [{
            ...
            "usuario": r["usuario"],
            "monto": round(r["monto"]),
            "usd": round(r["usd"]) if ("usd" in g.columns and pd.notna(r["usd"])) else None,
 ...
 * { box-sizing: border-box; }
 ...
 .usdamt { display: block; font-size: 11px; color: #a16207; font-weight: 600; margin-top: 1px; }
...
DATA.tree.forEach(function(f){
 ...
 var rows=f.filas.map(function(r){
  ...
  var usd=r.usd?'<span class="usdamt">US$ '+Math.round(r.usd).toLocaleString('es-AR')+'</span>':'';
  return '<div class="row"><span class="when">'+r.fecha+' '+r.hora+'</span><span><span class="cpt">'+esc(r.concepto)+'</span>'+asoc+'<br><span class="who">cargó: '+esc(r.usuario)+'  ·  mov '+r.mov+'  '+est+'</span></span><span class="ramt">'+fmtF(r.monto)+usd+'</span></div>';
 }).join('');
